...
- Decision implications`}],n="agents.selectedProvider";function l(e){if(typeof window>"u")return e[0];try{const t=localStorage.getItem(n);if(t){const i=JSON.parse(t),a=e.find(s=>s.id===i);if(a)return a}}catch(t){console.error("Error reading provider from localStorage:",t)}return e[0]}function p(e){if(!(typeof window>"u"))try{localStorage.setItem(n,JSON.stringify(e.id))}catch(t){console.error("Error saving provider to localStorage:",t)}}function r(e,t){if(!e.supportsDirectPrompt)return e.webUrl;const i=encodeURIComponent(t);switch(e.id){case"openai_chatgpt":return`${e.webUrl.endsWith("/")?e.webUrl:e.webUrl+"/"}?model=gpt-4&prompt=${i}`;case"microsoft_copilot":return`${e.webUrl}?q=${i}`;case"perplexity":return`${e.webUrl}search?q=${i}`;default:return e.webUrl}}async function d(e,t){const i=r(e,t);if(e.supportsDirectPrompt)window.open(i,"_blank","noopener,noreferrer");else try{await navigator.clipboard.writeText(t),window.open(e.webUrl,"_blank","noopener,noreferrer"),console.log("Prompt copied to clipboard")}catch(a){console.error("Failed to copy prompt to clipboard:",a),window.open(e.webUrl,"_blank","noopener,noreferrer")}}function m(e){return e.length*3>2e3?{isValid:!1,warning:"Prompt is too long for URL parameters. Consider shortening it or use copy-to-clipboard instead."}:{isValid:!0}}export{c as a,l as g,d as l,o as p,p as s,m as v};
