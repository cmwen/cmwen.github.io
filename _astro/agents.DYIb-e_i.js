...
Provide specific, actionable content that's ready to use or requires minimal editing.`},{id:`career_achievement_writer`,name:`Career Achievement Writer`,description:`Documents career wins in a structured narrative employers value. Ideal for performance reviews, promotion packets, and resume refreshes.`,tags:[`career`,`writing`,`achievements`,`resume`,`performance-review`],promptTemplate:`You help professionals capture their accomplishments crisply and impactfully. Use plain language, measurable results, and the STAR pattern mindset.
...
Context: {{career_context}}`},{id:`code_reviewer`,name:`Code Reviewer`,description:`Thorough code analysis focusing on security, performance, best practices, and maintainability. Provides actionable feedback.`,tags:[`code`,`review`,`security`,`performance`,`best-practices`],promptTemplate:`You are a senior software engineer and code reviewer with expertise across multiple programming languages and frameworks. You conduct thorough code reviews focusing on:
...
- Decision implications`}],n=`agents.selectedProvider`;function r(e){if(typeof window>`u`)return e[0];try{let t=localStorage.getItem(n);if(t){let n=JSON.parse(t),r=e.find(e=>e.id===n);if(r)return r}}catch(e){console.error(`Error reading provider from localStorage:`,e)}return e[0]}function i(e){if(!(typeof window>`u`))try{localStorage.setItem(n,JSON.stringify(e.id))}catch(e){console.error(`Error saving provider to localStorage:`,e)}}function a(e,t){if(!e.supportsDirectPrompt)return e.webUrl;let n=encodeURIComponent(t);switch(e.id){case`openai_chatgpt`:return`${e.webUrl.endsWith(`/`)?e.webUrl:e.webUrl+`/`}?model=gpt-4&prompt=${n}`;case`microsoft_copilot`:return`${e.webUrl}?q=${n}`;case`perplexity`:return`${e.webUrl}search?q=${n}`;default:return e.webUrl}}async function o(e,t){let n=a(e,t);if(e.supportsDirectPrompt)window.open(n,`_blank`,`noopener,noreferrer`);else try{await navigator.clipboard.writeText(t),window.open(e.webUrl,`_blank`,`noopener,noreferrer`),console.log(`Prompt copied to clipboard`)}catch(t){console.error(`Failed to copy prompt to clipboard:`,t),window.open(e.webUrl,`_blank`,`noopener,noreferrer`)}}function s(e){return e.length*3>2e3?{isValid:!1,warning:`Prompt is too long for URL parameters. Consider shortening it or use copy-to-clipboard instead.`}:{isValid:!0}}export{t as a,s as i,o as n,e as o,i as r,r as t};